...
random_concentration = np.random.randint(1, 10000, size=particle_radius.size)
...
for k, l in pair_indices:
    ...
    r_i_indices = rng.integers(
        0, number_in_bins[k], size=N_events, endpoint=False
    )
    r_j_indices = rng.integers(
        0, number_in_bins[l], size=N_events, endpoint=False
    )
    ...
    # Save gain and loss for coagulation events
...
# # bin particles
# particle_radius = np.sort(particle_radius)  # sort particles by size
# particles_original = particles.copy()
# number_in_bins, bins = np.histogram(particle_radius, bins=radius_bins)

# # Get the bin index for each particle
# # Note: np.digitize returns indices starting from 1,
# # so if you want to have 0-based indexing, subtract 1
# bin_indices = np.digitize(particle_radius, bins) - 1


# delta_t = 100  # time step in seconds

# total_number_concentration = 100000 * 1e6  # particles per m^3
# total_number_tracked = np.sum(number_in_bins)
# # simulated volume
# volume_sim = total_number_tracked / total_number_concentration
...
# # step from smallest to largest bin
# for k in range(bins.size - 1):
#     # step from all possible bins larger than k, to largest bin k+1
#     for l in range(k + 1):
#         # outside edge of the bin, kernel
#         Kmax = kernel_bins[k, l]
...
#             if number_in_bins[k] > 0 and number_in_bins[l] > 0:
#                 # Select random particles from bins k and l
#                 r_i = rng.integers(0, number_in_bins[k], endpoint=False)
#                 r_j = rng.integers(0, number_in_bins[l], endpoint=False)
#                 # get the index in the particle array where the bin starts
#                 index_r_i = np.searchsorted(bin_indices, k)
#                 index_r_j = np.searchsorted(bin_indices, l)

#                 # Early exit if particles sizes are zero, due
#                 # to previous coagulation
#                 if (
#                     particle_radius[index_r_i + r_i] == 0
#                     or particle_radius[index_r_j + r_j] == 0
#                 ):
#                     continue

#                 # Calculate the kernel for the selected particles
#                 B = interp_kernel.ev(
#                     particle_radius[index_r_i + r_i], particle_radius[index_r_j + r_j]
#                 )
#                 # B = kernel_helper.GetKernel(M[k][r_i], M[l][r_j])

#                 # Perform coagulation with probability B/Kmax
#                 probability_of_coagulation = B / Kmax
#                 if rng.random() < probability_of_coagulation:
#                     # If true coagulation event occurs
#                     new_radius = (
#                         particle_radius[index_r_i + r_i] ** 3
#                         + particle_radius[index_r_j + r_j] ** 3
#                     ) ** (1 / 3)

#                     # update gain and loss
#                     loss[index_r_i + r_i] = particle_radius[index_r_i + r_i]
#                     loss[index_r_j + r_j] = particle_radius[index_r_j + r_j]
#                     gain[index_r_j + r_j] = new_radius

#                     # update the particle array
#                     # set smaller particle to zero
#                     particle_radius[index_r_i + r_i] = 0
#                     # set larger particle to new radius
#                     particle_radius[index_r_j + r_j] = new_radius
