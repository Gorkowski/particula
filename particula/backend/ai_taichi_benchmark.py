...
GUIDE_PATH = Path(
    r"C:\GitHub\particula\particula\backend\taichi_benchmark_guide.md"
)
...
def convert(file_path: Path, prompt: str | None = None) -> None:
    ...
    if prompt is None:
        prompt = (
            f"Take the following python test file {file_path.name} "
            "and use it to help make a benchmark following the "
            "taichi_benchmark_guide.md. DO NOT modify"
            f" the test file {file_path.name} or the guide."
            " If the _benchmark.py file already in made, review it and "
