...
@ti.func
def fget_stokes_number(particle_inertia_time: ti.f64, kolmogorov_time: ti.f64) -> ti.f64:
...
def kget_stokes_number(
    ...
    for i in range(result.shape[0]):
        result[i] = fget_stokes_number(particle_inertia_time[i], kolmogorov_time[i])

@register("get_stokes_number", backend="taichi")
def get_stokes_number_taichi(particle_inertia_time, kolmogorov_time):
    """Public Taichi wrapper for Stokes number calculation."""
    if not (isinstance(particle_inertia_time, np.ndarray) and isinstance(kolmogorov_time, np.ndarray)):
        raise TypeError("Taichi backend expects NumPy arrays for both inputs.")

    pit, kt = np.atleast_1d(particle_inertia_time), np.atleast_1d(kolmogorov_time)
    n = pit.size
