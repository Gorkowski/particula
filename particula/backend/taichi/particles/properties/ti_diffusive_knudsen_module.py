...
def ti_get_diffusive_knudsen_number(
    ...
    for arg in (particle_radius, particle_mass, friction_factor):
    ...
    buf_sum   = ti.ndarray(ti.f64, shape_2d); buf_sum.from_numpy(sum_r)
    buf_m     = ti.ndarray(ti.f64, shape_2d); buf_m.from_numpy(red_m)
    buf_f     = ti.ndarray(ti.f64, shape_2d); buf_f.from_numpy(red_f)
    buf_cont  = ti.ndarray(ti.f64, shape_2d); buf_cont.from_numpy(cont_mat)
    buf_kin   = ti.ndarray(ti.f64, shape_2d); buf_kin.from_numpy(kin_mat)
    buf_res   = ti.ndarray(ti.f64, shape_2d)
