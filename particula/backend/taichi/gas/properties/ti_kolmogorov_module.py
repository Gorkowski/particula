...
def kget_kolmogorov_time(
    ...
    for i in range(result.shape[0]):
        result[i] = fget_kolmogorov_time(
            kinematic_viscosity[i], turbulent_dissipation[i]
        )
...
def kget_kolmogorov_length(
    ...
    for i in range(result.shape[0]):
        result[i] = fget_kolmogorov_length(
            kinematic_viscosity[i], turbulent_dissipation[i]
        )
...
def kget_kolmogorov_velocity(
    ...
    for i in range(result.shape[0]):
        result[i] = fget_kolmogorov_velocity(
            kinematic_viscosity[i], turbulent_dissipation[i]
        )
...
def ti_get_kolmogorov_time(kinematic_viscosity, turbulent_dissipation):
    ...
    a1, a2 = np.atleast_1d(kinematic_viscosity), np.atleast_1d(
        turbulent_dissipation
    )
    n = a1.size
