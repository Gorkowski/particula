...
def benchmark_antoine_vapor_pressure_csv():
    ...
    for array_length in ARRAY_LENGTHS:
        # ------------ input ------------------------------------------------
        temperature = random_generator.random(array_length, dtype=np.float64) * 200.0 + 250.0  # 250–450 K
        ...
        # ------------ Ti buffers -------------------------------------------
        constant_a_ti = ti.ndarray(dtype=ti.f64, shape=array_length); constant_a_ti.from_numpy(constant_a_array)
        constant_b_ti = ti.ndarray(dtype=ti.f64, shape=array_length); constant_b_ti.from_numpy(constant_b_array)
        constant_c_ti = ti.ndarray(dtype=ti.f64, shape=array_length); constant_c_ti.from_numpy(constant_c_array)
        temperature_ti = ti.ndarray(dtype=ti.f64, shape=array_length);  temperature_ti.from_numpy(temperature)
        result  = ti.ndarray(dtype=ti.f64, shape=array_length)
        # ------------ timing -----------------------------------------------
        stats_py  = get_function_benchmark(
            lambda: py_antoine(constant_a_value, constant_b_value, constant_c_value, temperature), ops_per_call=array_length
        )
        stats_ti  = get_function_benchmark(
            lambda: ti_antoine(constant_a_value, constant_b_value, constant_c_value, temperature), ops_per_call=array_length
        )
        stats_k   = get_function_benchmark(
            lambda: kget_antoine_vapor_pressure(constant_a_ti, constant_b_ti, constant_c_ti, temperature_ti, result),
    ...
    save_combined_csv(os.path.join(output_directory, "antoine_vapor_pressure_benchmark.csv"),
                      header, result_rows)
...
def benchmark_clausius_clapeyron_vapor_pressure_csv():
    result_rows: list[list] = []
    random_generator = np.random.default_rng(seed=RNG_SEED)
    latent_heat_value, temperature_initial_value, pressure_initial_value = 4.066e4, 373.15, 1.01325e5
    for array_length in ARRAY_LENGTHS:
        temperature = random_generator.random(array_length, dtype=np.float64) * 200.0 + 250.0
        ...
        latent_heat_ti = ti.ndarray(dtype=ti.f64, shape=array_length); latent_heat_ti.from_numpy(latent_heat_array)
        temperature_initial_ti = ti.ndarray(dtype=ti.f64, shape=array_length); temperature_initial_ti.from_numpy(temperature_initial_array)
        pressure_initial_ti = ti.ndarray(dtype=ti.f64, shape=array_length); pressure_initial_ti.from_numpy(pressure_initial_array)
        temperature_ti  = ti.ndarray(dtype=ti.f64, shape=array_length); temperature_ti.from_numpy(temperature)
        result   = ti.ndarray(dtype=ti.f64, shape=array_length)

        stats_py = get_function_benchmark(
            lambda: py_cc(latent_heat_value, temperature_initial_value, pressure_initial_value, temperature), ops_per_call=array_length
        )
        stats_ti = get_function_benchmark(
            lambda: ti_cc(latent_heat_value, temperature_initial_value, pressure_initial_value, temperature), ops_per_call=array_length
        )
        stats_k  = get_function_benchmark(
            lambda: kget_clausius_clapeyron_vapor_pressure(
                latent_heat_ti, temperature_initial_ti, pressure_initial_ti, temperature_ti, GAS_CONSTANT, result
            ),
            ops_per_call=array_length,
        )
        result_rows.append([array_length, *stats_py["array_stats"], *stats_ti["array_stats"],
                     *stats_k["array_stats"]])
    ...
    output_directory = os.path.join(os.path.dirname(__file__), "benchmark_outputs")
    os.makedirs(output_directory, exist_ok=True)
    save_combined_csv(os.path.join(output_directory, "clausius_clapeyron_benchmark.csv"),
                      header, result_rows)
    with open(os.path.join(output_directory, "system_info.json"), "w", encoding="utf-8") as fh:
        json.dump(get_system_info(), fh, indent=2)
    plot_throughput_vs_array_length(
...
def benchmark_buck_vapor_pressure_csv():
    ...
    for array_length in ARRAY_LENGTHS:
        ...
        stats_py = get_function_benchmark(lambda: py_buck(temperature), ops_per_call=array_length)
        stats_ti = get_function_benchmark(lambda: ti_buck(temperature), ops_per_call=array_length)
        stats_k  = get_function_benchmark(
            lambda: kget_buck_vapor_pressure(temperature_ti, result), ops_per_call=array_length
        )
        result_rows.append([array_length, *stats_py["array_stats"], *stats_ti["array_stats"],
                     *stats_k["array_stats"]])
    ...
    output_directory = os.path.join(os.path.dirname(__file__), "benchmark_outputs")
    os.makedirs(output_directory, exist_ok=True)
    save_combined_csv(os.path.join(output_directory, "buck_vapor_pressure_benchmark.csv"),
                      header, result_rows)
    with open(os.path.join(output_directory, "system_info.json"), "w", encoding="utf-8") as fh:
        json.dump(get_system_info(), fh, indent=2)
    plot_throughput_vs_array_length(
