...
def benchmark_dynamic_viscosity_csv():
    ...
    for array_length in ARRAY_LENGTHS:
        # ----- random input data -----------------------------------------
        temperatures = random_generator.random(array_length, dtype=np.float64) * 150.0 + 200.0   # 200-350 K
        reference_viscosity_array = np.full(array_length, REF_VISCOSITY_AIR_STP, dtype=np.float64)
        reference_temperature_array = np.full(array_length, REF_TEMPERATURE_STP, dtype=np.float64)
    ...
    # ---------------- header --------------------------------------------
    python_header  = ["python_"        + h for h in python_statistics["array_headers"]]
    taichi_header  = ["taichi_"        + h for h in taichi_statistics["array_headers"]]
    taichi_kernel_header  = ["taichi_kernel_" + h for h in taichi_kernel_statistics["array_headers"]]
    ...
    # ---------------- system info JSON ----------------------------------
    with open(os.path.join(output_directory, "system_info.json"), "w",
