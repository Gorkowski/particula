...
def phi(  # pylint: disable=too-many-positional-arguments
    ...
    **kwargs,
    ...
    particle_area = (
        ...
        else in_handling(particle_area, u.m**2)
    )

    molecular_enhancement_val = (
        mol_enh(**kwargs)
        if molecular_enhancement is None
        else in_scalar(molecular_enhancement)
    )

    vapor_attachment = (
        np.transpose([in_scalar(in_scalar(vapor_attachment)).m])
        * in_scalar(in_scalar(vapor_attachment)).u
    )

    vapor_speed_val = (
        cbar(**kwargs) / 4
        if vapor_speed is None
        else in_handling(vapor_speed, u.m / u.s)
    )
