...
def benchmark_aerodynamic_length_csv():
    """
    ...
    for n in ARRAY_LENGTHS:
        # ---------- random input data -----------------------------------
        pl  = rng.random(n, dtype=np.float64) * 1e-6 + 1e-9          # m
        psc = rng.random(n, dtype=np.float64)       + 1.0            # –
        asc = rng.random(n, dtype=np.float64)       + 1.0            # –
        rho = rng.random(n, dtype=np.float64) * 500  + 1000.0        # kg/m³
    ...
    # ------------ CSV ---------------------------------------------------
    csv_path = os.path.join(out_dir, "aerodynamic_length_benchmark.csv")
